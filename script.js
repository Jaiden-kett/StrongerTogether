...
document.addEventListener("DOMContentLoaded", function(){
    ...
    logInEnter("logInPassword");
    logInEnter("logInUsername");
    fillConfirmFields();    
    daysSober(); 
    recentUrge();
    disclaimerText();
...
function logInEnter(textBoxId){
    const textBox = document.getElementById(textBoxId);
    if(!textBox) return;

    if(textBoxId === "logInPassword"|| textBoxId === "logInUsername"){
...
function reasonForSoberText() {
    ...
    reasonForSoberText.textContent = `"${user.purpose}" -${user.firstName +' ' +  user.lastName}`;
}
